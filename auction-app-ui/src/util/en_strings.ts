const EN_STRINGS = {
  FOOTER: {
    ABOUT_US: 'About US',
    TERMS_AND_CONDITIONS: 'Terms and Conditions',
    PRIVACY_AND_POLICY: 'Privacy and policy',
    AUCTION: 'AUCTION',
    GET_IN_TOUCH: 'GET IN TOUCH',
    CALL_US: 'Call us at',
  },

  NAVBAR: {
    HOME: 'Home',
    SHOP: 'Shop',
    MY_ACCOUNT: 'My Account',
    INPUT_PLACEHOLDER: 'Try enter: Shoes',
  },

  HEADER: {
    LOGIN: 'Login',
    CREATE_ACCOUNT: 'Create Account',
  },

  HOME: {
    CATEGORIES: 'CATEGORIES',
    BID_NOW: 'Bid now',
    NEW_ARRIVALS: 'New Arrivals',
    LAST_CHANCE: 'Last Chance',
    HIGHLIGHTED_PRODUCT: 'Highlighted product',
    START_FROM: 'Start from',
  },

  SHOP: {
    SINGLE_PRODUCT: 'Single product',
  },

  SINGLE_PRODUCT: {
    STARTS_FROM: 'Starts from',
    HIGHEST_BID: 'Highest bid',
    NUMBER_OF_BIDS: 'Number of bids',
    TIME_LEFT: 'Time left',
    DETAILS: 'Details',
    SELLER_INFORMATION: 'Seller information',
    CUSTOM_REVIEWS: 'Custom reviews',
    PLACE_BID: 'PLACE BID',
    NO_BID_MESSAGE: 'Be first to bid for this product',
    INPUT_PLACEHOLDER_ENTER: 'Enter $',
    INPUT_PLACEHOLDER_HIGHER: 'or higher',
    INPUT_BID_ERROR: 'You must enter price first',
  },

  IMAGE_PICKER: {
    FOCUSED_IMAGED: 'Focused',
  },

  NOTIFICATION_BAR: {
    HIGHEST_BID_PLACED: 'Congrats! You are the highest bider!',
    OUTBIDDED: 'There are higher bids than yours. You could give a second try!',
    AUCTION_WON: 'Congratulations! You outbid the competition.',
    AUCTION_LOST: 'Unfortunately you have been outbidded!',
  },
};

export default EN_STRINGS;
